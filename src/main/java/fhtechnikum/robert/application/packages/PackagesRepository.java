...
public class PackagesRepository extends Repository {
    ...
    public boolean createCards(Card card, int package_id) {
        String query = "INSERT INTO cards (package_id, card_id, name, damage, monster_type, element_type, username) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection connection = Database.getConnection()) {
            assert connection != null;
            try (PreparedStatement stmt = connection.prepareStatement(query)){
